...
fn generate_resume_service(pci_addresses: &[String]) -> String {
    let remove_cmds: String = pci_addresses
        .iter()
        .map(|pci| format!(
            "  PCI=/sys/bus/pci/devices/{pci}; \\\n\
             \x20 [ -d \"$PCI\" ] && echo 1 > \"$PCI/remove\" 2>/dev/null || true;"
        ))
        .collect::<Vec<_>>()
        .join(" \\\n");

    format!(
        r#"# Hyprpier - Reset Thunderbolt/USB4 controllers on resume
# Generated by: hyprpier setup --resume

[Unit]
Description=Reset Thunderbolt/USB4 controllers on resume
...
{remove_cmds} \
  ...
  i=0; \
  while ! ls /sys/bus/thunderbolt/devices/[0-9]-0 >/dev/null 2>&1 && [ $i -lt 15 ]; do \
    sleep 1; \
    i=$((i+1)); \
  done; \
  sleep 2; \
...
pub fn install_resume_service() -> Result<()> {
    // Detect Thunderbolt/USB4 controller PCI addresses
    let pci_addresses = thunderbolt::get_controller_pci_addresses();

    if pci_addresses.is_empty() {
        bail!("No Thunderbolt/USB4 controller found. Is your system Thunderbolt-capable?");
    }

    for addr in &pci_addresses {
        println!("Detected Thunderbolt/USB4 controller at: {}", addr);
    }

    let service = generate_resume_service(&pci_addresses);
