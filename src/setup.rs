...
fn generate_resume_service(pci_address: &str) -> String {
    format!(
        r#"# Hyprpier - Reset Thunderbolt controller and USB devices on resume
# Generated by: hyprpier setup --resume

[Unit]
Description=Reset Thunderbolt controller and USB devices on resume
...
ExecStart=/bin/sh -c '\
  ...
  for xhci in /sys/bus/pci/drivers/xhci_hcd/????:??:??.?; do \
    if readlink -f "$xhci" | grep -q "{pci}"; then \
      addr=$(basename "$xhci"); \
      echo "$addr" > /sys/bus/pci/drivers/xhci_hcd/unbind 2>/dev/null || true; \
      sleep 0.5; \
      echo "$addr" > /sys/bus/pci/drivers/xhci_hcd/bind 2>/dev/null || true; \
    fi; \
  done'
